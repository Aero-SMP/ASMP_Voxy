...
final class NodeRequest {
    ...
    /** Restores a transaction journal entry, including whether a mesh was ever available. */
    int restoreMesh(int child, int mesh) {
        requireBit(child);
        int previous = this.meshes[child];
        this.meshes[child] = mesh;
        int bit = 1 << child;
        if (mesh == NodeManager.NULL_GEOMETRY_ID) this.meshMask &= ~bit;
        else this.meshMask |= bit;
        return previous;
    }
    ...
    /** Restores the exact pre-transaction existence state for renderer rollback. */
    void restoreChildExistence(int child, boolean present, byte existence) {
        requireBit(child);
        int bit = 1 << child;
        this.childExistence[child] = existence;
        if (present) this.existenceMask |= bit;
        else this.existenceMask &= ~bit;
    }
